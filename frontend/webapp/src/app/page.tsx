...
const buildApiConfig = (options: ApiConfigOptions = {}): ApiConfig => {
  ...
  const resolvedBase =
    primaryBase || (options.requirePreferred ? null : fallbackBase)
  ...
  const buildUrl = (endpoint: string) =>
    new URL(joinPath(apiPrefix, endpoint), base)
...
const resolveSensorTypes = async (
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Sensor types fetch failed: ${res.status} - ${text.substring(0, 200)} (URL: ${sensorTypesUrl})`,
    )
  }
  ...
  if (!codes.length) {
    throw new Error(
      'Sensor types endpoint returned no codes. لطفاً مطمئن شوید SensorType در Django تعریف شده است.',
    )
  }
...
const latestReadingsFetcher = async ([
  ...
  const sensorTypes =
    sensorTypesOverride ||
    (await resolveSensorTypes(config, configuredSensorTypes))
  ...
  if (!res.ok) {
    const text = await res.text()
    throw new Error(
      `Dashboard API error: ${res.status} - ${text.substring(0, 200)} (URL: ${target})`,
    )
  }
...
const hierarchyFetcher = async (
  path: string,
): Promise<FarmHierarchyResponse> => {
  ...
  if (!res.ok) {
    const text = await res.text()
    throw new Error(
      `Farm hierarchy error: ${res.status} - ${text.substring(0, 200)} (URL: ${target})`,
    )
  }
...
async function activeAlertsFetcher(path: string): Promise<AlertLog[]> {
  ...
  if (!res.ok) {
    const text = await res.text()
    throw new Error(
      `Active alerts error: ${res.status} - ${text.substring(0, 200)} (URL: ${target})`,
    )
  }
...
export default function HomePage() {
  ...
  const [selectedBarnId, setSelectedBarnId] = React.useState<number | null>(
    null,
  )
  const [selectedZoneId, setSelectedZoneId] = React.useState<number | null>(
    null,
  )
  ...
  React.useEffect(() => {
    if (selectedFarm?.barns?.length) {
      if (
        !selectedBarnId ||
        !selectedFarm.barns.some((barn) => barn.id === selectedBarnId)
      ) {
  ...
  React.useEffect(() => {
    if (selectedBarn?.zones?.length) {
      if (
        !selectedZoneId ||
        !selectedBarn.zones.some((zone) => zone.id === selectedZoneId)
      ) {
  ...
  const { data, error } = useSWR(
    selectedZoneSensorTypes.length
      ? ([
          ...
          selectedZoneSensorTypes.join(','),
        ] as LatestReadingsKey)
      : null,
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-2">
        {sensors.map((sensor) => (
          ...
          >
            {sensor.name}{' '}
            <span className="text-slate-400">({sensor.sensor_type.code})</span>
          </span>
        ))}
      </div>
    )
      ...
      <div className="w-full max-w-6xl space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div>
              <h2 className="text-xl font-semibold">
                ساختار مزرعه → بارن → زون
              </h2>
              <p className="text-sm text-slate-400">
                ابتدا مزرعه را انتخاب کنید تا بارن‌ها، زون‌ها و سنسورهای هر بخش
                نمایش داده شود.
              </p>
            </div>
            <span className="text-xs text-slate-500">
              به‌روزرسانی هر ۳۰ ثانیه از مسیر dashboard/farm-hierarchy/
            </span>
          </div>
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-xs text-slate-400">
                        سنسورها: {farm.sensor_count}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-slate-500">
                      تعداد بارن‌ها: {farm.barns.length}
                    </p>
                  </button>
                ))}
              </div>
                ...
                <div className="space-y-3 rounded-lg border border-slate-800/80 bg-slate-950/40 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="text-lg font-semibold">
                      بارن‌های {selectedFarm.name}
                    </h3>
                    <span className="text-xs text-slate-400">
                      تعداد سنسورها: {selectedFarm.sensor_count}
                    </span>
                  </div>
                  ...
                  {selectedFarm.sensors.length > 0 && (
                    <div>
                      <p className="text-xs text-slate-400">
                        سنسورهای ثبت‌شده برای خود مزرعه (بدون بارن/زون):
                      </p>
                  ...
                  {selectedFarm.barns.length ? (
                      ...
                      {selectedFarm.barns.map((barn) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-xs text-slate-400">
                              سنسورها: {barn.sensor_count}
                            </span>
                          </div>
                          <p className="mt-1 text-xs text-slate-500">
                            تعداد زون‌ها: {barn.zones.length}
                          </p>
                          {barn.description && (
                            <p className="mt-1 text-xs text-slate-500">
                              {barn.description}
                    ...
                    <p className="text-sm text-slate-500">
                      برای این مزرعه هنوز بارنی ثبت نشده است.
                    </p>
                  )}
                    ...
                    <div className="space-y-3 rounded-lg border border-slate-800 bg-slate-900/40 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <h4 className="text-base font-semibold">
                          زون‌های {selectedBarn.name}
                        </h4>
                        <span className="text-xs text-slate-400">
                          سنسورها: {selectedBarn.sensor_count}
                        </span>
                      </div>
                      ...
                      {selectedBarn.zones.length ? (
                        ...
                        <p className="text-sm text-slate-500">
                          زون ثبت نشده است؛ سنسورها در سطح خود بارن هستند.
                        </p>
                      )}
                    </div>
                  )}
        ...
        {!error && selectedFarm && !selectedFarmHasSensors && (
          <div className="rounded-lg border border-slate-800 bg-slate-900/60 px-4 py-3 text-sm text-slate-200">
            برای مزرعه انتخاب‌شده هیچ سنسوری ثبت نشده است؛ لطفاً مزرعه دیگری را
            انتخاب کنید.
          </div>
        )}
              ...
              {activeAlerts.map((alert) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-red-100/70 mt-1">
                    سنسور: {alert.sensor ?? 'نامشخص'} | مقدار:{' '}
                    {alert.reading_value ?? '—'} | زمان:{' '}
        ...
        <p className="mt-4 text-center text-xs text-slate-500">
          داده‌ها از FastAPI (یا به صورت مستقیم از Django) از مسیر{' '}
          <code>dashboard/latest-readings/</code> روی <code>BASE_URL</code>
          واکشی می‌شود. اگر <code>NEXT_PUBLIC_SENSOR_TYPES</code> را ست نکنید،
          فرانت‌اند ابتدا <code>sensor-types/</code> را می‌خواند و به صورت
          خودکار کد سنسورها را از Django استخراج می‌کند؛ در غیر این صورت مقدار
          متغیر را (مثلاً <code>temp_sensor,ammonia</code>) استفاده می‌کند. در
          صورت نیاز می‌توانید پیشوندی مثل <code>api/v1</code> را هم در{' '}
          <code>NEXT_PUBLIC_API_PREFIX</code> بگذارید تا مسیر کامل مانند{' '}
          <code>/api/v1/dashboard/latest-readings/?sensor_types=...</code> ساخته
          شود. اکنون کارت‌ها فقط پس از انتخاب زون (و وجود سنسور در آن زون) نمایش
          داده می‌شوند. برای ساختار مزرعه نیز از{' '}
          <code>dashboard/farm-hierarchy/</code> استفاده می‌شود و انتخاب
          مزرعه/بارن/زون در بالا مسیر نظارت را شفاف می‌کند.
        </p>
      </div>
    </main>
  )
