...
def _rule_matches_value(rule: LiveStatusRule, value: float) -> bool:
...
def evaluate_live_status_for_reading(reading: SensorReading) -> Iterable[LiveStatus]:
    """
    بررسی قوانین فعال و ثبت وضعیت زنده در صورت نقض آستانه.
    ...
    rules = LiveStatusRule.objects.filter(is_active=True).filter(
        models.Q(sensor=sensor) | models.Q(sensor_type=sensor_type)
    )

    triggered: list[LiveStatus] = []
    ...
    for rule in rules:
        ...
        status = LiveStatus.objects.create(
            rule=rule,
            farm=device.farm,
            barn=device.barn,
            zone=device.zone,
